...
function StoryOne(msg)
    ...
    -- 记录给茉莉发送消息后返回的选项编号
    -- ! -1%10==9
    local OptionReturn = getUserConf(msg.fromQQ, "isStory1Option1Choice3", -1) %
                             10
    -- 剩余行动轮 初始为4
    ...
    if (Option == 0) then
        ...
        elseif (MainIndex == 13) then
            ...
            content = content .. "\f注意：当前剩余行动次数" ..
                          string.format("%.0f", actionRoundLeft) .. "/4"
    ...
    elseif (Option == 3) then
        MainIndex = 31
        -- ! 准备跳转回家
        if (ChoiceIndex == 8) then return MessageSent(msg, OptionReturn) end
        content = Story1[MainIndex][ChoiceIndex]
        if (ChoiceIndex == 6) then
            -- 第一次解锁商店
            if (getUserConf(msg.fromQQ, "isShopUnlocked", 0) == 0) then
                content = content .. "\f{FormFeed}{FormFeed}" ..
                              "重要消息：『商店』已经解锁！输入指令“进入商店”来进入商品界面\f系统消息：您得到了500FL"
...
function MessageSent(msg, OptionReturn)
    ...
    if (OptionReturn ~= 9) then
        -- 添加过渡句
        content =
            "你带着还不错的心情回到家中，茉莉此时似乎还没回来，不过稍等片刻，" ..
                Story1[22]
...
function GoToMessage(msg, index, ChoiceIndex)
    ...
    if (ChoiceIndex == 2 or ChoiceIndex == 4) then
        content = content .. "{FormFeed}{FormFeed}{FormFeed}" ..
...
function ReturnLastOption(msg, MainIndex, ChoiceIndex, Border, OptionReturn)
    ...
    if (MainIndex == 8) then
        if (ChoiceIndex == 1) then
            content = content .. "{FormFeed}{FormFeed}" ..
                          Story1[MainIndex][ChoiceIndex + 1] ..
        ...
        elseif (ChoiceIndex == 4) then
            content = content .. "{FormFeed}{FormFeed}{FormFeed}" ..
                          ...
                          Story1[MainIndex][ChoiceIndex + 2] ..
                          "{FormFeed}{FormFeed}{FormFeed}" ..
                          Story1[MainIndex][ChoiceIndex + 3]
    ...
    elseif (MainIndex == 9) then
        if (ChoiceIndex == 1) then
            ...
            setUserConf(msg.fromQQ, "ChoiceIndex", ChoiceIndex + 2)
            ChoiceIndex = ChoiceIndex + 2
        elseif (ChoiceIndex == 5 or ChoiceIndex == 7 or ChoiceIndex == 9 or
            ChoiceIndex == 11 or ChoiceIndex == 13 or ChoiceIndex == 15 or
            ChoiceIndex == 17) then
            content = content .. "{FormFeed}{FormFeed}{FormFeed}" ..
                          Story1[MainIndex][ChoiceIndex + 1]
            setUserConf(msg.fromQQ, "ChoiceIndex", ChoiceIndex + 1)
    ...
    elseif (MainIndex == 10) then
        if (ChoiceIndex == 1 or ChoiceIndex == 6 or ChoiceIndex == 8 or
            ChoiceIndex == 10 or ChoiceIndex == 13) then
            content = content .. "{FormFeed}{FormFeed}{FormFeed}" ..
                          Story1[MainIndex][ChoiceIndex + 1]
            setUserConf(msg.fromQQ, "ChoiceIndex", ChoiceIndex + 1)
            ChoiceIndex = ChoiceIndex + 1
...
function JudgeActionRound(msg)
    ...
    if (actionRoundLeft >= 1) then return true end
    return false
end
...
function StoryOneChoose(msg, res)
    ...
    if (Option == 1 or Option == 11 or Option == 12) then
        if (res * 1 == 3 and
            getUserConf(msg.fromQQ, "isStory1Option1Choice3", -1) ~= -1) then
            return "该选项处于不可选中状态哦~"
        end
    end
