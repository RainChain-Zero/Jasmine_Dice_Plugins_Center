...
function StoryZero(msg)
    local MainIndex, Option, Choice, ChoiceIndex, ChoiceSelected =
        GetUserConf(msg.fromQQ, {"MainIndex", "Option", "Choice", "ChoiceIndex", "ChoiceSelected0"}, {1, 0, 0, 1, 0})
    local content
    -- 判断是否进入分支
    if (Option == 0) then
        -- 选项1
    ...
    elseif (Option == 1) then
        ...
        if (Choice == 1) then
            ...
            content = Story0[MainIndex]
            MainIndex = 7
            SetUserConf(msg.fromQQ, {"MainIndex", "Option", "Choice"}, {MainIndex, 0, 0})
            return content
        elseif (Choice == 2) then
            ...
            if (ChoiceIndex > 2) then
                SetUserConf(msg.fromQQ, {"MainIndex", "ChoiceIndex", "Option", "Choice"}, {7, 1, 0, 0})
            end
            return content
    ...
    elseif (Option == 2) then
        ...
        if (Choice == 1) then
            ...
            content = Story0[8]
            SetUserConf(msg.fromQQ, {"MainIndex", "Choice"}, {7, 0})
            return content
        elseif (Choice == 2) then
            content = Story0[9]
            Init(msg)
            if (GetUserConf(msg.fromQQ, "isStory0Read", 0) == 0) then
                SetUserConf(msg.fromQQ, {"梦的开始", "isStory0Read"}, {1, 1})
