...
function favor_punish(msg)
    ...
    local _year, _month, _day, _hour =
        getUserConf(msg.fromQQ, "year_last", 2021),
        getUserConf(msg.fromQQ, "month_last", 10),
        getUserConf(msg.fromQQ, "day_last", 11),
        getUserConf(msg.fromQQ, "hour_last", 23)
    ...
    -- 分段降低好感
    if (subday <= 3) then
        if (subday == 1 and subhour <= -15) then return "" end
        if (favor < 8500 and favor > 1250) then
            Llimit, Rlimit = 80 * math.log(2 * subday, 2),
                             100 * math.log(2 * subday, 2)
        elseif (favor >= 8500) then
            Llimit, Rlimit = 150 * math.log(2 * subday, 2),
                             180 * math.log(2 * subday, 2)
        else
            Llimit, Rlimit = 30 * math.log(2 * subday, 2),
                             35 * math.log(2 * subday, 2)
        end
    elseif (subday <= 8) then
        Llimit, Rlimit = 180 * subday, 200 * subday
    else
        Llimit, Rlimit = 820 + 215 * (subday - 5) *
                             math.log(2 * (subday - 5), 2), 870 + 245 *
                             (subday - 5) * math.log(2 * (subday - 5), 2)
    end
    --
    -- ! 道具减免
    Llimit, Rlimit = Llimit * (1 - favorTimePunishDownRate(msg)),
                     Rlimit * (1 - favorTimePunishDownRate(msg))
    -- //todo 将左右端点取整才可带入ranint
    Llimit, Rlimit = math.modf(Llimit), math.modf(Rlimit)
    favor = favor - ranint(Llimit, Rlimit)
    if (favor < 500 and flag == true) then favor = 500 end
...
function add_favor_food(favor)
    ...
    -- return 100
    -- 随机好感上升,低好感用户翻倍
    if (favor <= 1250) then return ranint(40, 60) end
    return ranint(20, 30)
end
...
function blackList(msg)
    ...
    if (favor <= -300 and favor > -600) then
        sendMsg(
            "Warning:检测到{nick}的好感度过低，即将触发机体下限保护机制！",
            msg.fromGroup, msg.fromQQ)
        sendMsg("Warning:检测到用户" .. msg.fromQQ .. "好感度过低" ..
                    "在群" .. msg.fromGroup, 0, 2677409596)
    end
    if (favor < -600) then
        eventMsg(".admin blackqq " .. "违反人机和谐共处条例 " ..
                     msg.fromQQ, 0, 2677409596)
        eventMsg(".group " .. msg.fromGroup .. " ban " .. msg.fromQQ .. " " ..
                     tostring(-favor), msg.fromGroup, getDiceQQ())
        return "已触发！"
    end
    return ""
end
...
function rcv_food(msg)
    ...
    if (cnt >= 4 or cnt < 0) then return "参数有误请重新输入哦~" end
    ...
    if (today_sorry == 0) then
        ...
        --
        -- //!递归调用实现多次喂食
        if (cnt > 0) then rcv_food(msg) end
        -- if(flag_food==cntT)then
        --     flag_food=0
        cnt = 0
        return
            "你眼前一黑，手中的食物瞬间消失，再看的时候，眼前的烧酒口中还在咀嚼着什么，扭头躲开了你的目光\n今日已收到投喂" ..
                topercent(self_today_gift) .. "kg\n累计投喂" ..
                topercent(self_total_gift) .. "kg"
        -- end
    else
        if (msg.fromQQ == "2677409596") then
            setUserToday(msg.fromQQ, "hug_needed_to_sorry", 1) -- 设定需要抱茉莉以道歉的次数
            return
                "#咀嚼声 哼...唔，主人可别认为这样我就会原谅你！#扭捏着 抱、抱我！"
        else
            if (favor >= 1500) then
                setUserToday(msg.fromQQ, "hug_needed_to_sorry", 1) -- 设定需要抱茉莉以道歉的次数
                return
                    "好、好吃！...不！不对！你如果不抱我的话我绝不原谅你！#撇过头"
            else
                setUserToday(msg.fromQQ, "rude", 0)
                return
                    "哼，行吧，茉莉这次就原谅你，下次记得别这样了啊，茉莉我可是很宽容的~#笑"
            end
        end
    end
end
...
function rcv_Ciallo_morning_master(msg)
    ...
    local today_sorry = getUserToday(msg.fromQQ, "sorry", 0)
    -- 关键词匹配
    local judge = msg.fromMsg == "早" or
                      string.find(msg.fromMsg, "早上好", 1) ~= nil or
                      string.find(msg.fromMsg, "早啊", 1) ~= nil or
            ...
            if (favor >= 1200) then
                if (today_rude <= 2 and today_sorry <= 1) then
                    if (hour >= 5 and hour <= 10) then
                        ...
                        return
                            "诶诶诶{nick}早上好！今天是来找茉莉玩的吗？"
                    else
                        return
                            "唔...{nick}难道是在和另一个自己对话吗...因为现在怎么看都不是早上的样子..."
                    end
                end
            end
        end
...
function rcv_Ciallo_afternoon(msg)
    ...
    if (hour >= 18 or hour <= 6) then
        return
            "茉莉这次才不会搞错呢！才不会被{nick}这种小花招骗到！外面明明那么黑（指着窗外）"
    end
    -- 爱酱特殊问候模式
    if (msg.fromQQ == "2677409596") then
        if (today_rude <= 3 and today_sorry <= 1) then
            return
                "午安我的主人~你不在的时间里茉莉会照顾好自己的哟"
        end
    end
    if (today_rude <= 2 and today_sorry <= 1) then
        if (favor < 1500) then
            return
                "嗯？要睡午觉了吗，也是，养好精神也很重要呢"
        elseif (favor < 4000) then
            return "午安哦，茉莉也有点困了...呼呼呼"
        elseif (favor < 6000) then
            return
                "诶要睡了吗，好、好吧...之后记得找茉莉玩哦"
        else
            return
                "嗯呐，在你午睡的时候，请让茉莉在一旁陪着你吧#依"
        end
    end
end
...
function rcv_Ciallo_noon(msg)
    ...
    if (msg.fromQQ == "2677409596") then
        if (today_rude >= 4 or today_sorry >= 2) then
            return
                "怎么了——笨↗蛋↘主人，茉莉现在，不！想！理！你！"
        else
            if (hour >= 11 and hour <= 14) then
                return
                    "主人中午好呀！茉、茉莉吃得...有点饱了...呼呼呼#倒床上"
            else
                return
                    "唔姆，#抬头看窗外 主人你没事吧 睡傻了吗（上来捏脸）"
            end
        end
    else
        if (today_rude >= 3 or today_sorry >= 2) then
            return
                "Error!机体故障！目标信息丢失，无法识别该对象！你是谁啊茉莉不认识你"
        else
            if (hour >= 11 and hour <= 14) then
                if (favor <= 1500) then
                    return
                        "唔，中午好！{nick}，吃过午饭了吗？吃过就赶快去休息吧"
                elseif (favor <= 4500) then
                    return
                        "中午好呀{nick}——今天过去一半了哦，有什么要做的就抓紧吧"
                elseif (favor <= 6000) then
                    return
                        "中，中午好{nick}，是有什么要和茉莉说吗！"
                else
                    return
                        "中↘午↗好——呀！想睡觉了呢...在那之前#拉衣角 再陪茉莉玩一会吧"
                end
            else
                return
                    "咦，现在，是中午？好吧，既然{nick}这么说，那么，中午好！"
            end
        end
    end
end
    ...
    if (msg.fromQQ == "2677409596") then
        if (today_rude <= 3 and today_sorry <= 1) then
            if (hour >= 11 and hour <= 14) then
                ...
                return
                    "诶？中午好...？唔 好吧 原来这时间叫做中午...茉莉记下了，毕竟我最相信主人了嘛"
            end
        end
    else
        if (today_rude <= 2 and today_sorry <= 1) then
            if (favor >= 1200) then
                if (hour >= 11 and hour <= 14) then
                    return
                        "诶，中午好？是…在和茉莉说吗，应该……是吧"
                else
                    return
                        "唔..可现在不是中午哦？不过 茉莉也向你问号哦~#踮起脚尖打招呼"
                end
            end
        end
    end
    ...
    if (msg.fromQQ == "2677409596") then
        if (today_rude <= 3 and today_sorry <= 1) then
            if ((hour >= 21 and hour <= 23) or (hour >= 0 and hour <= 4)) then
                ...
                return
                    "晚安哦我的主人，茉莉今天明天也会一直喜欢你的！"
            else
                return
                    "主——人——！不要捉弄茉莉，现在显然不是睡觉时间啦！"
            end
        end
    else
        if (today_rude <= 2 and today_sorry <= 1) then
...
function night_master(msg)
    ...
    if (msg.fromQQ == "2677409596") then
        if (today_rude <= 3 and today_sorry <= 1) then
            if ((hour >= 21 and hour <= 23) or (hour >= 0 and hour <= 4)) then
                return
                    "主人晚安！！诶...主人你说不是对我说的...？呜...#委屈"
            else
                return
                    "主人这是睡傻——了吗，现在明显还没到睡觉时间呢"
            end
        end
    else
        if (favor >= 2000) then
            if (today_rude <= 2 and today_sorry <= 1) then
                if ((hour >= 21 and hour <= 23) or (hour >= 0 and hour <= 4)) then
                    return
                        "{sample:晚安哦，虽然不知道为什么，但茉莉想主动对你说晚安~|希望明天我们能依然保持赤诚和热爱|晚安，茉莉会你身边安心陪你睡着的哦？|晚安~愿你梦中星河烂漫，美好依旧}"
                else
                    return
                        "嗯...{nick}现在好像还没到晚安的时间呢..."
                end
            end
        end
    end
...
function Ciallo_night_2(msg)
    ...
    if (msg.fromQQ == "2677409596") then
        if (today_rude <= 3 and today_sorry <= 1) then
            if ((hour >= 21 and hour <= 23) or (hour >= 0 and hour <= 4)) then
                return
                    "诶？主人真的会睡吗...茉莉很怀疑哦，但还是晚安啦"
            elseif (hour >= 12 and hour <= 15) then
                return
                    "唔 看来到睡午觉的时间了呢 主人请好好休息吧"
            else
                return
                    "（叹气）果然...主人脑子里只有睡觉吗...现在可不是该睡觉的时间"
            end
        end
    else -- 其他用户根据好感判断回复
        if (today_rude <= 2 and today_sorry <= 1) then
...
function punish_favor_rude(msg)
    ...
    if (today_sorry == 1) then
        ...
        return
            "你！你不是才向茉莉道完歉吗！你、你这个鬼畜！茉莉今天绝对不会理你了！"
    end
    -- 每rude一次减65好感度
    setUserConf(msg.fromQQ, "好感度", favor - 65)
    local blackReply = blackList(msg)
...
rude_table = {
    "爬", "(爬", "爪 巴", "cnm", "nm", "rnm", "tmd", "滚", "（爪",
    "傻逼", "傻比", "煞笔", "煞比", "sb", "wdnmd", "操", "我操"
}
...
function say_sorry(msg)
    ...
    if (today_sorry >= 2) then
        return
            "哼！知错不改的坏孩子！茉莉今天绝对不会理你的！"
    end
    -- 对爱酱的判定
    if (msg.fromQQ == "2677409596") then
        if (today_rude <= 0) then
            return
                "诶诶诶？！主人为什么要道歉啊 不不、不会是茉莉做了什么坏事吧！"
        elseif (today_rude <= 3) then
            ...
            setUserToday(msg.fromQQ, "hug_needed_to_sorry", 1) -- 设定需要抱茉莉以道歉的次数
            return
                "唔姆姆...既然主人都这么说了...茉莉其实也没有那么生气啦#撇过头，抱抱我就原谅你了！"
        else
            today_sorry = today_sorry + 1
            setUserToday(msg.fromQQ, "sorry", today_sorry)
            setUserToday(msg.fromQQ, "gifts",
                         getUserToday(msg.fromQQ, "gifts", 0) - 1) -- 减少已投喂次数以腾出次数给道歉投喂
            return
                "哼，笨蛋主人，现在才想起来和我道歉吗！不行不行！...茉莉、茉莉要吃的！"
        end
    else
        if (today_rude <= 0) then
            return
                "诶？！怎、怎么了 为什么要无端向茉莉道歉啊#慌乱"
        elseif (today_rude <= 2) then
            today_sorry = today_sorry + 1
            setUserToday(msg.fromQQ, "sorry", today_sorry)
            return
                "...好、好吧，只要你答应茉莉不会再犯就好！茉莉可是很宽容的#叉腰"
        else
            today_sorry = today_sorry + 1
            setUserToday(msg.fromQQ, "sorry", today_sorry)
            setUserToday(msg.fromQQ, "gifts",
                         getUserToday(msg.fromQQ, "gifts", 0) - 1) -- 减少已投喂次数以腾出次数给道歉投喂
            return
                "哼...就算你这么说了...但茉莉可不会这么轻易原谅你（沉默）我、我肚子饿了..."
        end
    end
end
...
function interaction(msg)
    ...
    if (not RS_judge) then return "" end
    local level
    if (favor <= 1500) then
        level = "less"
    ...
    local first, second = "", string.match(msg.fromMsg,
                                           "^[%s]*[%S]*[%s]*[%S]*$",
                                           #normal_order_old + 1)
    first, second = string.match(second, "^[%S]*"), string.match(second,
                                                                 "^[%S]*",
                                                                 string.find(
                                                                     second, " ") +
                                                                     1)
    if (first ~= "互动") then return "" end
    ...
    for k, v in pairs(reply) do
        if (k == flag) then return v[ranint(1, #v)] end
    end
end
...
function _Ciallo_normal(msg)
    ...
    if (msg.fromMsg == "茉莉") then flag = true end
...
function action(msg)
    ...
    if (judge_hug) then
        ...
        if (msg.fromQQ == "2677409596") then
            ...
            if (today_rude >= 3 or today_sorry >= 2) then
                ...
                reply_main =
                    "哼！做了这种事的坏孩子不要碰茉莉！#有力挣开"
            else
                if (hugtosorry == 1) then
                    setUserToday(msg.fromQQ, "rude", 0)
                    setUserToday(msg.fromQQ, "hug_needed_to_sorry", 0)
                    reply_main =
                        "唔姆姆，茉莉这次、这次...这次就原谅你！#音量莫名提高"
                else
                    if (favor <= 1500) then
                        if (today_hug <= today_hug_limit) then
    ...
    if (judge_touch) then
        ...
        if (msg.fromQQ == "2677409596") then
            if (today_rude >= 4 or today_sorry >= 2) then
                ...
                reply_main =
                    "唔唔唔，主、主人不要摸啦，头、头发会乱的...#闭眼缩起脖子"
            end
        else
            if (today_rude >= 3 or today_sorry >= 2) then
                setUserConf(msg.fromQQ, "好感度", favor - 100)
                reply_main =
                    "不 不要！你是坏人，茉莉的头才不会让你摸呢！"
            else
                if (favor <= 1000) then
                    if (today_touch <= today_touch_limit) then
    ...
    if (judge_lift) then
        ...
        if (msg.fromQQ == "2677409596") then
            if (today_rude <= 3 and today_sorry <= 1) then
                ...
                reply_main =
                    "啊主主主、主人 好、好高啊！再、再转几圈吧！#露出了开心的笑容"
            else
                setUserConf(msg.fromQQ, "好感度", favor - 100)
    ...
    if (judge_kiss) then
        ...
        if (msg.fromQQ == "2677409596") then
            if (today_rude <= 3 and today_sorry <= 1) then
                ...
                reply_main =
                    "啊...主、主人...你你你！#低头脸红 你讨厌死了！#捶胸口时埋到怀里"
            else
                setUserConf(msg.fromQQ, "好感度", favor - 150)
                reply_main =
                    "笨蛋主人！#快速扭过头然后看你 茉莉原谅你之前绝对不会让你亲的！"
            end
        else
            if (today_rude <= 2 and today_sorry <= 1) then
                if (favor <= 1700) then
    ...
    if (judge_hand) then
        ...
        if (msg.fromQQ == "2677409596") then
            if (today_rude <= 3 and today_sorry <= 1) then
                ...
                reply_main =
                    "诶？要牵手吗...嗯...嗯 那 就不要放开了哦~主人——"
            else
                reply_main =
                    "哼...茉莉可还没有原谅主人哦，所以，不给你——牵！"
            end
        else
            if (today_rude <= 2 and today_sorry <= 1) then
                if (favor <= 1200) then
    ...
    -- action 捏/揉脸
    local judge_face = string.find(msg.fromMsg, "捏脸", 1) ~= nil or
                           string.find(msg.fromMsg, "揉脸", 1) ~= nil or
    ...
    if (judge_face) then
        ...
        if (msg.fromQQ == "2677409596") then
            ...
            if (today_rude <= 2 and today_sorry <= 1) then
                ...
                reply_main =
                    "不要随便碰我！你这个坏人！大笨蛋！#耍脾气"
            end
        end
    end
    -- 赞美和情感表达系统
    -- 可爱
    local judge_cute = string.find(msg.fromMsg, "可爱", 1) ~= nil or
                           string.find(msg.fromMsg, "卡哇伊", 1) ~= nil or
    ...
    if (judge_cute) then
        ...
        if (msg.fromQQ == "2677409596") then
            if (today_rude <= 3 and today_sorry <= 1) then
                ...
                reply_main =
                    "诶诶诶？主...主人夸我了！#惊喜 还..还有点不好意思呢...#傻笑"
            else
                reply_main =
                    "哼，不管主人怎么夸，茉莉都不会心动的 #气鼓鼓嘟起嘴"
            end
        else
            if (today_rude <= 2 and today_sorry <= 1) then
                if (favor <= 1050) then
    ...
    -- express suki
    local judge_suki = string.find(msg.fromMsg, "喜欢", 1) ~= nil or
                           string.find(msg.fromMsg, "suki", 1) ~= nil
    if (judge_suki) then
        ...
        if (msg.fromQQ == "2677409596") then
            if (today_rude <= 3 and today_sorry <= 1) then
                ...
                reply_main =
                    "啊..#呆住 Error！检测到机体温度迅速升高，要主人抱抱才能缓解！"
            else
                reply_main =
                    "哼...就算主人这么说了...不！不对！主人是大笨蛋！茉莉才不会因为这种花言巧语而心软呢！"
            end
        else
            if (today_rude <= 2 and today_sorry <= 1) then
                if (favor <= 1500) then
    ...
    -- express love
    local judge_love = string.find(msg.fromMsg, "爱", 1) ~= nil or
                           string.find(msg.fromMsg, "love", 1) ~= nil
    if (judge_love and not judge_cute) then
        ...
        if (msg.fromQQ == "2677409596") then
            if (today_rude <= 3 and today_sorry <= 1) then
                ...
                reply_main =
                    "啊啊啊主主主主主人你你你突然说些什么啊...我我我...茉莉...茉莉当然...也爱你啦（逐渐小声）"
            else
                reply_main =
                    "诶？爱...主人爱我...？#面无表情但脸红 可、可别以为这样茉莉就会原谅你...#移开视线"
            end
        else
            if (today_rude <= 2 and today_sorry <= 1) then
                if (favor <= 1800) then
                ...
                reply_main =
                    "哼，茉莉可不想被茉莉说爱我，不、不然...不然茉莉不也是笨蛋了吗..."
            end
        end
    end
    -- 最后判断是否是“互动--部位”格式
...
function action_main(msg)
    ...
    if (reply_main ~= "") then return reply_main end
    action(msg)
    if (reply_main ~= "") then return reply_main end
    _Ciallo_normal(msg)
    return reply_main
...
function setfavor(msg)
    local first, second = "", ""
    first, second = string.match(msg.fromMsg, "^[%s]*(%d*)[%s]*(%d*)$",
                                 #admin_order1 + 1)
    if (first == "" or second == "") then return "请输入正确的参数×" end
    if (msg.fromQQ == "3032902237" or msg.fromQQ == "2677409596" or msg.fromQQ ==
        "2225336268") then
        setUserConf(first * 1, "好感度", second * 1)
...
function time(msg)
    if (msg.fromQQ == "3032902237" or msg.fromQQ == "2677409596" or msg.fromQQ ==
        "2225336268") then
        return month .. "月" .. day .. "日" .. hour .. "时" .. minute ..
                   "分"
    end
end
msg_order["当前时间"] = "time"
...
function favor_history(msg)
    ...
    if (msg.fromQQ == "3032902237" or msg.fromQQ == "2677409596" or msg.fromQQ ==
        "2225336268") then
        return "目标最后一次好感交互在" ..
                   string.format("%.0f", getUserConf(QQ, "year_last", 2021)) ..
                   "年" ..
                   string.format("%.0f", getUserConf(QQ, "month_last", 10)) ..
                   "月" ..
                   string.format("%.0f", getUserConf(QQ, "day_last", 11)) ..
                   "日" ..
                   string.format("%.0f", getUserConf(QQ, "hour_last", 23)) ..
                   "时" .. "\n好感度为" ..
                   string.format("%.0f", getUserConf(QQ, "好感度", 0))
    end
end
...
function table_draw(tab) return tab[ranint(1, #tab)] end