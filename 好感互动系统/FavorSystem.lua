...
function Notice(msg)
    ...
    if (favorUVersion ~= 45) then
        SetUserConf(msg.fromQQ, {"noticeQQ", "favorVersion"}, {0, 45})
    end
...
function favorTimePunishDownRate(msg)
    ...
    elseif (GetUserConf(msg.fromQQ, "favorTimePunishDownDDLFlag", 1) == 0) then
        ...
        -- 更新标记，下次不做提醒
        SetUserConf(msg.fromQQ, {"favorTimePunishDownDDLFlag", "favorTimePunishDownRate"}, {1, 0})
    end
    return 0
end
...
function favor_punish(msg)
    ...
    SetUserConf(msg.fromQQ, {"month_last", "day_last", "hour_last", "year_last"}, {month, day, hour, year})
...
function SyncFlag(msg)
    ResetSyncFlag(msg)
end
msg_order["重置数据同步"] = "SyncFlag"
