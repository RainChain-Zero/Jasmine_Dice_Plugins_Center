...
function Trade(msg)
    local content = ""
    -- //? 首先判断是不是被交易方的回复指令
    if (string.find(msg.fromMsg, "同意") == nil and
        string.find(msg.fromMsg, "接受") == nil and
        string.find(msg.fromMsg, "拒绝") == nil) then
        -- //? 判断被请求方是否在同一个群中
        ...
        QQReceive, itemRequestNum, itemRequest, itemReceiveNum, itemReceive =
            string.match(msg.fromMsg,
                         "^[%s]*[%[CQ:at,qq=]*(%d*)[%]]*[%s]*[T,t]*[%s]*(%d*)[%s]*(%S*)[%s]*[F,f]*[%s]*(%d*)[%s]*(%S*)$",
                         #trade_order + 1)
        if ((QQReceive == nil or QQReceive == "") or
            (itemRequestNum == nil or itemRequestNum == "") or
            (itemRequest == nil or itemRequest == "") or
            (itemReceiveNum == nil or itemReceiveNum == "") or
            string.find(msg.fromMsg, "@") ~= nil) then
        ...
        if (not flag1 or not flag2) then
            return
                "系统：检测到您输入了数据库中不存在的道具呢"
        end
        -- ! 禁止好感交易
        if (string.find(itemRequest, "好感") ~= nil or
            string.find(itemReceive, "好感") ~= nil) then
            return "重要数据不能进行交易哦？~"
        end
        local itemRequestNow = getUserConf(msg.fromQQ, itemRequest, 0)
        if (itemRequestNow - itemRequestNum * 1 < 0) then
            return "系统：您的" .. itemRequest ..
                       "余量不足，无法发起交易"
        end
        ...
        if (itemReceiveNum * 1 == 0) then
            if (isInGroup == 1) then
                content = "[CQ:at,qq=" .. QQReceive .. "]\n" ..
                              "系统：您收到来自用户编号为" ..
                              msg.fromQQ .. "的赠送——您得到了" ..
                              itemRequestNum .. itemRequest
            else
                content = "系统：您收到来自" .. "用户编号为" ..
                              msg.fromQQ .. "的赠送——您得到了" ..
                              itemRequestNum .. itemRequest
            end
            setUserConf(msg.fromQQ, itemRequest, getUserConf(msg.fromQQ,
                                                             itemRequest, 0) -
                            itemRequestNum)
            setUserConf(QQReceive, itemRequest,
                        getUserConf(QQReceive, itemRequest, 0) + itemRequestNum)
        ...
        local QQReceive1, QQReceive2, QQRequest1, QQRequest2 = "0", "0", "0",
                                                               "0"
        ...
        if (isInGroup == 1) then
            content = "[CQ:at,qq=" .. QQReceive .. "]\n" ..
                          "系统：您收到来自用户编号为" ..
                          msg.fromQQ .. "的交易请求——您将得到" ..
                          itemRequestNum .. itemRequest .. "；同时失去" ..
                          itemReceiveNum .. itemReceive ..
                          "\n是否接受？（输入 “交易同意/拒绝”）"
            sendMsg(content, msg.fromGroup, QQReceive)
        else
            content = "系统：您收到来自用户编号为" .. msg.fromQQ ..
                          "的交易请求——您将得到" .. itemRequestNum ..
                          itemRequest .. "；同时失去" .. itemReceiveNum ..
                          itemReceive ..
                          "\n是否接受？（输入 “交易 同意/拒绝”）"
            sendMsg(content, 0, QQReceive)
            sendMsg(
                "系统：您的交易请求已发送，请等待对方回复，结果将通过私聊通知。若长时间未回复，可能是对方未加本机好友并且所在群禁止了临时会话",
                msg.fromGroup, msg.fromQQ)
        end
        return ""
    elseif (string.find(msg.fromMsg, "同意") ~= nil or
        string.find(msg.fromMsg, "接受") ~= nil or
        string.find(msg.fromMsg, "拒绝") ~= nil) then
        -- debug
        -- return msg.fromQQ
        -- 记录获取交易发起方
        ...
        if (getUserConf(msg.fromQQ, "isQQBiggerThanNine", "n") == "y") then
            tradeRequest = string.format("%.0f", getUserConf(msg.fromQQ,
                                                             "tradeRequest1", 0)) ..
                               string.format("%.0f", getUserConf(msg.fromQQ,
                                                                 "tradeRequest2",
                                                                 "0"))
        else
            tradeRequest = string.format("%.0f", getUserConf(msg.fromQQ,
        ...
        if (getUserConf(tradeRequest, "isQQBiggerThanNine", "n") == "y") then
            tradeReceiveNow = string.format("%.0f", getUserConf(tradeRequest,
                                                                "tradeReceive1",
                                                                0)) ..
                                  string.format("%.0f", getUserConf(
                                                    tradeRequest,
                                                    "tradeReceive2", "0"))
        else
            tradeReceiveNow = string.format("%.0f", getUserConf(tradeRequest,
        ...
        if (reply == "同意" or reply == "接受") then
            ...
            if (itemReceiveNow - itemReceiveNum < 0) then
                ...
                return "系统：您的" .. itemReceive ..
                           "余量不足,交易已关闭"
            end

            -- 物品数量变更
            setUserConf(msg.fromQQ, itemReceive, itemReceiveNow - itemReceiveNum)
            setUserConf(tradeRequest, itemReceive, getUserConf(tradeRequest,
                                                               itemReceive, 0) +
                            itemReceiveNum)
            local itemRequest = getUserConf(tradeRequest, "itemRequest", "nil")
            local itemRequestNum =
                getUserConf(tradeRequest, "itemRequestNum", 0)
            local itemRequestNow = getUserConf(tradeRequest, itemRequest, 0)
            setUserConf(msg.fromQQ, itemRequest, getUserConf(msg.fromQQ,
                                                             itemRequest, 0) +
                            itemRequestNum)
            setUserConf(tradeRequest, itemRequest,
                        itemRequestNow - itemRequestNum)
...
function adminGift(msg)
    ...
    -- 目标 赠送道具数量 赠送道具 附加信息
    local QQ, num, item, message = string.match(msg.fromMsg,
                                                "[%s]*(%d*)[%s]*(%d*)[%s]*(%S*)[%s]*(.*)",
                                                #admin_order_gift + 1)
...
function Check()
    ...
    for k, _ in pairs(Item) do
        ...
        if (flag1 and flag2) then return true, true end
    end
    return flag1, flag2
end
