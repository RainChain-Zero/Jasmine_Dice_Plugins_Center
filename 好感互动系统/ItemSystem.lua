...
function UseItem(msg)
    local reply =
        "唔姆姆，你这是要对着空气做什么呀？（部分物品需要赠送给茉莉才会触发：“赠送茉莉 数量 道具”数量不填默认为1）"
    local num = ""
    num, item = string.match(msg.fromMsg, "[u,U][%s]*(%d*)[%s]*(.*)")
    if (item == nil or item == "") then
        return
            "请输入道具名哦~（输入“道具图鉴”可查看目前支持的所有道具）"
    end
    -- 数量默认为1
...
function GiveGift(msg)
    local num = ""
    num, item =
        string.match(msg.fromMsg, "[%s]*(%d*)[%s]*(.+)", #gift_order + 1)
    if (num == "" or num == nil) then num = 1 end
    if (item == "" or item == nil) then
        return
            "诶诶诶？{nick}这是要送给茉莉什么呀？是...？惊喜吗！"
    end
    -- 合理性判断
    ...
    if (not flag2) then return "好像该礼物的剩余数量不足哦" end
    if (num * 1 > 1 and item == "彩虹糖") then
        num = 1
        sendMsg(
            "注意，该道具具有随机效果，一次只能赠送一个哦",
    ...
    if (item ~= "推理小说" and item ~= "袋装曲奇") then
        setUserConf(msg.fromQQ, "好感度", getUserConf(msg.fromQQ,
                                                         "好感度", 0) + num *
                        1 * Gift_list[item].favor)
    elseif (item == "推理小说") then

        -- !时间惩罚降低的好感减少百分之多少，同类不覆盖
        local rate = getUserConf(msg.fromQQ, "favorTimePunishDownRate", 0)
        -- 更新时间，取最新时间
        setUserConf(msg.fromQQ, "favorTimePunishDownDDL",
                    os.time() + 5 * 24 * 60 * 60 * 1000)
        -- ! 打上标记，用做发送提醒的标记
    ...
    elseif (item == "袋装曲奇") then
        ...
        setUserConf(msg.fromQQ, "addFavorDDL_Cookie",
                    os.time() + 3 * 24 * 60 * 60 * 1000)
        -- ! 打上标记，用做发送提醒的标记
...
function UseCheck(msg, num, table)
    ...
    -- 不存在直接返回
    if (not flag1) then return false, false end
    -- 判断道具余量
...
function UnlockStory(msg, entryStoryCheck, item)
    if (entryStoryCheck == 1) then
        if (string.find(item, "梦的开始") ~= nil) then
            ...
            return
                "这把钥匙似乎和眼前的光芒产生了某种共鸣，倏忽间，光芒如同被某种强大的引力吸引般瞬间汇聚于钥匙上后逐渐稳定了下来...\f" ..
                    "系统：注意，剧情模式第一章已经解锁！"
        else
            return
                "你小心翼翼地将它向那团光球接近，但就要在你触及之时，一股强大的斥力将你远远弹开了..."
        end
    end
end
...
function SearchItem(msg)
    ...
    if (not flag) then return "该道具暂未被图鉴收录哦~" end

    local res = getUserConf(msg.fromQQ, item, 0)
    sendMsg("系统：正在检索..." .. ranint(20, 50) .. "%..." ..
                ranint(51, 80) .. "%...", msg.fromGroup, msg.fromQQ)
    sleepTime(1000)
    return "您目前的『" .. item .. "』数量为" ..
               string.format("%0.f", res) .. "\n(" .. Item[item] .. ")"
end
