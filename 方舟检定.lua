...
function ark_check(msg)
   ...
   --提取参数
   objTemp,num,sign,add = string.match(str,"[%s]*([^%s^%d^%+^%-]*)[%s]*(%d*)[%s]*([%+|%-]?)[%s]*(%d*)",#order_kng+1)
   --判断合法性
   ...
   if ((type(num)=="string" and (num=="" or num==nil)) and (type(add)=="string" and (add=="" or add==nil))) then
      ...
      --存在即时检定但无修订值
   elseif (type(num)=="string" and(num~="" and num~=nil) and (type(add)=="string" and (add=="" or add==nil))) then
      num=num*1+SpecialItem()
      --不存在即时检定但有修订值
   elseif (type(num)=="string" and(num=="" or num==nil) and (type(add)=="string" and (add~="" and add~=nil))) then
      if(sign=="+")then
...
function SpecialItem()
   for _,v in pairs(ItemsNeed2D10)
   do
      if(obj==v)then
...
function NormalJudge()
   if (res <= 5) then
      reply = "{pc}进行ark的" .. obj .. "检定:" .. "\nD100" .. "=" ..
      string.format("%.0f", res) .. "/" ..
      string.format("%.0f", num) .. "{strCriticalSuccess}"
   ...
   elseif (res <= num-25) then
      reply = "{pc}进行ark的" .. obj .. "检定:" .. "\nD100" .. "=" ..
      string.format("%.0f", res) .. "/" ..
      string.format("%.0f", num-25) .."{strRollHardSuccess} "
   elseif (res <= num-10) then
      reply = "{pc}进行ark的" .. obj .. "检定:" .. "\nD100" .. "=" ..
      string.format("%.0f", res) .. "/" ..
      string.format("%.0f", num-10) .."是较难成功，还可以吧"   
   elseif (res <= num) then
      reply = "{pc}进行ark的" .. obj .. "检定:" .. "\nD100" .. "=" ..
      string.format("%.0f", res) .. "/" ..
      string.format("%.0f", num) .."{strRollRegularSuccess}"
   elseif (res<=95) then
      reply = "{pc}进行ark的" .. obj .. "检定:" .. "\nD100" .. "=" ..
      string.format("%.0f", res) .. "/" ..
      string.format("%.0f", num) .."{strRollFailure}"
   else
      reply =  "{pc}进行" .. obj .. "检定:" .. "\nD100" .. "=" ..
      string.format("%.0f", res) .. "/" ..
      string.format("%.0f", num) .. "{strRollFumble}"
   end
   return reply
end
...
ItemsNeed2D10=
{
   "搏斗","拳术","暗器","刀剑","杖术","枪术","软兵","盾术","重钝器","弓弩","投掷","射击","炮术","爆破","兵械操作","无人机操作"
}
...
Hardness=
{
   ...
   function ()
      ...
      elseif (res<=95) then
         reply = "{pc}进行ark的较难" .. obj .. "检定:" .. "\nD100" .. "=" ..
         string.format("%.0f", res) .. "/" ..
         string.format("%.0f", num-10) .."{strRollFailure}"
      else
         reply = "{pc}进行ark的较难" .. obj .. "检定:" .. "\nD100" .. "=" ..
         string.format("%.0f", res) .. "/" ..
         string.format("%.0f", num-10) .. "{strRollFumble}"
      end
      return reply
   end,
   ["困难"]=
   function ()
      ...
      elseif (res<=95) then
         reply = "{pc}进行ark的困难" .. obj .. "检定:" .. "\nD100" .. "=" ..
         string.format("%.0f", res) .. "/" ..
         string.format("%.0f", num-25) .."{strRollFailure}"
      else
         reply = "{pc}进行ark的困难" .. obj .. "检定:" .. "\nD100" .. "=" ..
         string.format("%.0f", res) .. "/" ..
         string.format("%.0f", num-25) .. "{strRollFumble}"   
      end
      return reply
   end,
   ["极难"]=
   function ()
      ...
      elseif (res<=95) then
         reply = "{pc}进行ark的极难" .. obj .. "检定:" .. "\nD100" .. "=" ..
         string.format("%.0f", res) .. "/" ..
         string.format("%.0f", num-40) .."{strRollFailure}"
      else
         reply = "{pc}进行ark的极难" .. obj .. "检定:" .. "\nD100" .. "=" ..
         string.format("%.0f", res) .. "/" ..
         string.format("%.0f", num-40) .. "{strRollFumble}"
      end
      return reply
   end,
   ["普通"]=NormalJudge,
